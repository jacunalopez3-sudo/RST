(() => {
  ...
  function build() {
    ...
    clientWrap.insertAdjacentHTML('afterbegin', '<label>Cliente registrado</label><select id="catalogClient"><option value="">Seleccioneâ€¦</option><option value="manual">+ Cliente no registrado</option></select>');
    ...
    $('reporteForm').insertAdjacentHTML('afterbegin', '<input type="hidden" id="catalogClientId"><input type="hidden" id="catalogEquipmentId"><div class="btns"><button type="button" id="catalogRefresh">Actualizar catÃ¡logo</button><span id="catalogStatus" class="muted">CatÃ¡logo no disponible sin conexiÃ³n</span></div>');
  ...
  function label(item) { return `${item.equipment_name || '-'} â€” ${[item.brand, item.model].filter(Boolean).join(' ') || '-'} â€” Serie: ${item.serial_number || '-'} â€” Activo: ${item.asset_number || '-'}`; }
  ...
  async function loadLocal() { allClients = await BennuCatalog.clients(); const select = $('catalogClient'), current = select.value; select.innerHTML = ''; select.add(option('Seleccioneâ€¦', '')); for (const item of allClients) select.add(option(item.name, item.id)); select.add(option('+ Cliente no registrado', 'manual')); if ([...select.options].some(item => item.value === current)) select.value = current; }
  const formatDate = value => value ? new Date(value).toLocaleString('es-CR') : 'CatÃ¡logo no disponible sin conexiÃ³n';
  async function refresh() { try { const result = await BennuCatalog.sync(); await loadLocal(); $('catalogStatus').textContent = 'CatÃ¡logo actualizado: ' + formatDate(result.at); } catch (_) { $('catalogStatus').textContent = 'No se pudo actualizar; se mantiene la copia local'; } }
  async function init(options) { if (ready) return; ready = true; db = options.supabase; BennuCatalog.init({ supabase: db }); build(); await loadLocal(); $('catalogStatus').textContent = 'CatÃ¡logo actualizado: ' + formatDate(await BennuCatalog.lastSync()); if (navigator.onLine) refresh(); window.addEventListener('online', () => BennuCatalog.flushPending().then(refresh).catch(() => {})); }
  ...
  async function prepare(data) {
    ...
    const choice = window.prompt('Se detectaron cambios en un equipo registrado.\n\n1 = Solo este reporte\n2 = Actualizar catÃ¡logo\n3 = Cancelar', '1');
    if (choice === null || choice.trim() === '3') { const error = new Error('Guardado cancelado'); error.code = 'CATALOG_CANCELLED'; throw error; }
    ...
    if (replacesProtected && !confirm('El equipo ya tiene una serie o activo registrado.\nÂ¿Confirma que desea reemplazarlo?')) { const error = new Error('Guardado cancelado'); error.code = 'CATALOG_CANCELLED'; throw error; }
    if (!navigator.onLine) return BennuCatalog.prepareReport(data);
    try { await BennuCatalog.updateExistingEquipment({ equipmentId: data.equipmentId, clientId: data.clientId, original: data.originalEquipment, changes, createdAt: new Date().toISOString() }); alert('Equipo actualizado en el catÃ¡logo.'); }
    catch (error) { if (error.code === 'EQUIPMENT_CONFLICT') { alert(error.message + '\nLos cambios se guardarÃ¡n Ãºnicamente en este reporte.'); return BennuCatalog.prepareReport(data); } throw error; }
